...
class OneEuroFilter:
    ...
    def filter(self, x: float, t: float = None, axis: int = 0) -> float:
        ...
        # Estimate derivative
        dx = (x - self.x_prev) / dt
        dx_hat = self._exponential_smoothing(
            dx, self.dx_prev, self._alpha(self.d_cutoff, dt)
        )
        self.dx_prev = dx_hat
    ...
    def _exponential_smoothing(
        self, x_curr: float, x_prev: float, alpha: float
    ) -> float:
...
def create_smoother(cfg: dict[str, Any]):
    ...
    if kind == "one_euro":
        ...
        return MultiDimensionalFilter(
            ExponentialMovingAverage, alpha=cfg.get("alpha", 0.3)
        )
    else:
