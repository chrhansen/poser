...
class MetricsPlotter:
    ...
    def generate_offline_graph(self, csv_path: str, output_path: Optional[str] = None):
        ...
        # Plot data
        plt.plot(df['timestamp_ms'], df['knee_distance'], 
                label='Knee Distance', color='blue', linewidth=2)
        plt.plot(df['timestamp_ms'], df['ankle_distance'], 
                label='Ankle Distance', color='red', linewidth=2)
    ...
    def init_realtime_plot(self):
        ...
        self.knee_line, = self.ax.plot([], [], 'b-', label='Knee Distance', linewidth=2)
        self.ankle_line, = self.ax.plot([], [], 'r-', label='Ankle Distance', linewidth=2)
        
        # Setup plot
        self.ax.set_xlabel('Time (ms)', fontsize=12)
        self.ax.set_ylabel('Distance', fontsize=12)
        self.ax.set_title('Real-time Knee and Ankle Distances', fontsize=14, fontweight='bold')
    ...
    def update_realtime_plot(self, timestamp_ms: float, 
